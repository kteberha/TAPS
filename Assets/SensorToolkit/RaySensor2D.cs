...
namespace SensorToolkit
{
	...
	public class RaySensor2D : Sensor
    {
        ...
        void testRay()
        {
            var canDetectMultiple = !layerMaskIsSubsetOf(DetectsOnLayers, ObstructedByLayers) && (IgnoreList == null || IgnoreList.Length == 0);
            clearDetectedObjects();
            if (!canDetectMultiple && (IgnoreList == null || IgnoreList.Length == 0))
            {
                testRaySingle();
            }
            else
